...
from machine.config import (
    IO_PRINTER_ADDR_BASE,
    IO_PRINTER_ADDR_COUNT,
    IO_KEYBOARD_ADDR_BASE,
    IO_KEYBOARD_ADDR_COUNT,
)
from machine.cpu import CPU
from machine.bus import create_io_bus
...
def Machine(
    ...
    cpu = CPU(rom, ram, iobus_clk, iobus_ctrl, iobus_addr, iobus_data)
    ...
    dev0 = IODevPrinter(
        ...
        iobus_addr,
        iobus_data,
        address=IO_PRINTER_ADDR_BASE,
        address_count=IO_PRINTER_ADDR_COUNT,
        output=io_output,
        simulate_delay=io_output_delay,
    )
    dev1 = IODevKeyboard(
        iobus_clk,
        iobus_ctrl,
        iobus_addr,
        iobus_data,
        address=IO_KEYBOARD_ADDR_BASE,
        address_count=IO_KEYBOARD_ADDR_COUNT,
        source=io_input,
        simulate_delay=io_input_delay,
    )
