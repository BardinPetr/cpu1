...
class SwitchTransformer(Transformer):
    ...
    def __convert_to_if(alu: Tree, bit: int, cases: Dict, exit_label: Location):
        ...
        branches = [
            ...
            if isinstance(cases[br], dict)
            else (cases[br][0].children if cases[br] else [])
            for br in range(2)
        ]
    ...
    @v_args(inline=True)
    def instr_switch(
        self, alu: Tree, pos_end: int, pos_start: int, body: Dict[int, Dict | List]
    ):
